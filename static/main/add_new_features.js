...
function initTrueFalse() {
    ...
    html += `<button class="tile" onclick="${statement.answer === true ? `speakText('Correct!'); addStars(5, 'truefalse')` : `speakText('Try again')`}" style="padding:20px 30px;font-size:1.3rem;background:#6ee7b7;color:#333;border:3px solid #27ae60;border-radius:10px;cursor:pointer">✓ True</button>`;
    html += `<button class="tile" onclick="${statement.answer === false ? `speakText('Correct!'); addStars(5, 'truefalse')` : `speakText('Try again')`}" style="padding:20px 30px;font-size:1.3rem;background:#ff6b6b;color:white;border:3px solid #c92a2a;border-radius:10px;cursor:pointer">✗ False</button>`;
    html += `</div>`;
...
function initPhonics() {
    ...
    let html = `<div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(60px, 1fr));gap:8px">`;
    letters.forEach(letter => {
        html += `<button class="tile" onclick="speakText('${letter}. ${sounds[letter]}'); event.target.style.transform='scale(1.2)'; setTimeout(()=>event.target.style.transform='scale(1)',200)" style="padding:10px;font-size:1rem;font-weight:700;cursor:pointer;transition:all 0.3s;border:3px solid #ffd166;border-radius:8px;background:#fffacd">
            ${letter}
        </button>`;
    });
    html += `</div>`;
...
function initShapeSorting() {
    ...
    html += `<div style="border:3px dashed #ff6b6b;padding:20px;text-align:center;min-height:120px;border-radius:10px"><h4 style="margin:0 0 10px 0">🔴 Circles</h4><div id="circleTarget" style="min-height:60px"></div></div>`;
    html += `<div style="border:3px dashed #ffd166;padding:20px;text-align:center;min-height:120px;border-radius:10px"><h4 style="margin:0 0 10px 0">🟩 Squares</h4><div id="squareTarget" style="min-height:60px"></div></div>`;
    html += `<div style="grid-column:1/-1;display:flex;gap:10px;justify-content:center;flex-wrap:wrap;margin:20px 0">`;
    html += `<button class="tile" onclick="moveToCategory(this, 'circle')" style="font-size:2rem;cursor:pointer;border:3px solid #999;border-radius:8px;padding:10px 15px">🔴</button>`;
    html += `<button class="tile" onclick="moveToCategory(this, 'square')" style="font-size:2rem;cursor:pointer;border:3px solid #999;border-radius:8px;padding:10px 15px">🟩</button>`;
    html += `<button class="tile" onclick="moveToCategory(this, 'circle')" style="font-size:2rem;cursor:pointer;border:3px solid #999;border-radius:8px;padding:10px 15px">⭕</button>`;
    html += `<button class="tile" onclick="moveToCategory(this, 'square')" style="font-size:2rem;cursor:pointer;border:3px solid #999;border-radius:8px;padding:10px 15px">⬜</button>`;
    html += `</div>`;
...
function initSpotDifference() {
    let html = `<div style="grid-column:1/-1;text-align:center;font-weight:700;color:#ff6b6b;margin-bottom:10px">🔍 Find what's different!</div>`;
